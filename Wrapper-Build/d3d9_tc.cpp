// TEST C: No static local FrameLimiter - use global pointer
...
typedef int (WINAPI* PFN_PEB)(D3DCOLOR, LPCWSTR);
typedef int (WINAPI* PFN_PEE)();
typedef void (WINAPI* PFN_PESM)(D3DCOLOR, LPCWSTR);
typedef void (WINAPI* PFN_PESR)(D3DCOLOR, LPCWSTR);
typedef BOOL (WINAPI* PFN_PEQR)();
typedef void (WINAPI* PFN_PESO)(DWORD);
typedef DWORD (WINAPI* PFN_PEGS)();
static PFN_D3DC9 pD3DC9=0; static PFN_D3DC9Ex pEx=0;
static PFN_PEB pB=0; static PFN_PEE pE=0; static PFN_PESM pSM=0;
static PFN_PESR pSR=0; static PFN_PEQR pQR=0; static PFN_PESO pSO=0; static PFN_PEGS pGS=0;
typedef HRESULT (STDMETHODCALLTYPE *PFN_Pres)(IDirect3DDevice9*,const RECT*,const RECT*,HWND,const RGNDATA*);
typedef HRESULT (STDMETHODCALLTYPE *PFN_CD)(IDirect3D9*,UINT,D3DDEVTYPE,HWND,DWORD,D3DPRESENT_PARAMETERS*,IDirect3DDevice9**);
static PFN_Pres g_OrigPresent=0; static PFN_CD g_OrigCreateDevice=0;
...
static HRESULT STDMETHODCALLTYPE Hook_Present(IDirect3DDevice9* d,const RECT* sr,const RECT* dr,HWND hw,const RGNDATA* rg){
    if (g_pFL) { g_pFL->WaitForFrame(); }
    HRESULT hr = g_OrigPresent(d,sr,dr,hw,rg);
    if (g_pFL) { g_pFL->OnFramePresented(); }
    return hr;
}
...
static HRESULT STDMETHODCALLTYPE Hook_CreateDevice(IDirect3D9* d9,UINT a,D3DDEVTYPE t,HWND hw,DWORD fl,D3DPRESENT_PARAMETERS* pp,IDirect3DDevice9** out){
    if (g_pFL && pp && g_pFL->GetForceVSync()) {
        pp->PresentationInterval = D3DPRESENT_INTERVAL_ONE;
        pp->FullScreen_RefreshRateInHz = 0;
    }
    if (g_pFL && pp && pp->FullScreen_RefreshRateInHz > 0) {
        g_pFL->SetRefreshRate((double)pp->FullScreen_RefreshRateInHz);
    }
...
static void InitD3D9(){
    ...
    if(!g_hRealD3D9)return;
    pD3DC9=(PFN_D3DC9)GetProcAddress(g_hRealD3D9,"Direct3DCreate9");
    pEx=(PFN_D3DC9Ex)GetProcAddress(g_hRealD3D9,"Direct3DCreate9Ex");
    pB=(PFN_PEB)GetProcAddress(g_hRealD3D9,"D3DPERF_BeginEvent");
    pE=(PFN_PEE)GetProcAddress(g_hRealD3D9,"D3DPERF_EndEvent");
    pSM=(PFN_PESM)GetProcAddress(g_hRealD3D9,"D3DPERF_SetMarker");
    pSR=(PFN_PESR)GetProcAddress(g_hRealD3D9,"D3DPERF_SetRegion");
    pQR=(PFN_PEQR)GetProcAddress(g_hRealD3D9,"D3DPERF_QueryRepeatFrame");
    pSO=(PFN_PESO)GetProcAddress(g_hRealD3D9,"D3DPERF_SetOptions");
    pGS=(PFN_PEGS)GetProcAddress(g_hRealD3D9,"D3DPERF_GetStatus");
}
...
BOOL WINAPI DllMain(HINSTANCE h,DWORD r,LPVOID){
    if(r==DLL_PROCESS_ATTACH){
        ...
        _wfopen_s(&g_logFile,lp,L"w"); if(g_logFile){fputs("TestC: no static local\n",g_logFile);fflush(g_logFile);}
        InitializeCriticalSection(&g_logCS);
        g_pFL = &FrameLimiter::Get(); // init static local once
        Log("TestC: FL init done\n");
    }else if(r==DLL_PROCESS_DETACH){
        if(g_logFile){fclose(g_logFile);g_logFile=0;} DeleteCriticalSection(&g_logCS);
    }
    return TRUE;
}
...
HRESULT WINAPI Direct3DCreate9Ex(UINT v,IDirect3D9Ex**e){InitD3D9();if(!pEx)return E_FAIL;HRESULT hr=pEx(v,e);if(SUCCEEDED(hr)&&e&&*e)HookD3D9Vtable((IDirect3D9*)*e);return hr;}
