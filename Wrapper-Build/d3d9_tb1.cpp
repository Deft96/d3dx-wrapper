// TEST B1: Hook_Present: just m_firstFrame = false
...
static CRITICAL_SECTION g_testCS;
static volatile bool g_testCSInit = false;

static HRESULT STDMETHODCALLTYPE Hook_Present(IDirect3DDevice9* d,const RECT* sr,const RECT* dr,HWND hw,const RGNDATA* rg){
    if (!g_testCSInit) { InitializeCriticalSection(&g_testCS); g_testCSInit = true; }
    EnterCriticalSection(&g_testCS); LeaveCriticalSection(&g_testCS);
...
BOOL WINAPI DllMain(HINSTANCE h,DWORD r,LPVOID){
    if(r==DLL_PROCESS_ATTACH){
        ...
        _wfopen_s(&g_logFile,lp,L"w"); if(g_logFile){fputs("TestB1\n",g_logFile);fflush(g_logFile);}
